<template>
  ...
  <div class="My_post">
    ...
    <div v-else-if="posts.length === 0">You haven't submitted any posts yet.</div>
    <div v-else>
  ...
  setup() {
        ...
        if (userSnap.exists()) {
          ...
          posts.value = allPosts
            ...
            .map(doc => ({
              id: doc.id,
              ...doc.data(),
              type: foundSnap.docs.some(d => d.id === doc.id) ? 'Found' : 'Lost'
            }))
        }
      } catch (error) {
    ...
    const deletePost = async (item) => {
      try {
        ...
        const collectionName = item.type === 'Found' ? 'foundItems' : 'lostItems'
        ...
        const updatedPosts = (userData.posts || []).filter(postId => postId !== item.id)
